...
async def execute_resolve(
    ...
    if domain:
        ...
        target_domain = settings.DOMAIN_NAME if host_domain in ("test", "localhost", "127.0.0.1") else host_domain
    ...
    if redirect_obj.expired_on:
        ...
        exp_on = redirect_obj.expired_on if redirect_obj.expired_on.tzinfo else redirect_obj.expired_on.replace(tzinfo=timezone.utc)
        if now >= exp_on:
...
async def resolve_redirect_query(
    ...
    return await execute_resolve(request, target_slug, domain, db, user_agent, x_forwarded_for)
