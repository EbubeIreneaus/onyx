...
<template>
  <div class="space-y-6 max-w-6xl mx-auto pb-12">
    ...
    <div
      v-if="loading"
      class="py-24 flex justify-center items-center"
    >
      <UIcon
    ...
    <div
      v-else-if="!data"
      class="py-16 text-center space-y-4 bg-zinc-900/50 rounded-2xl border border-zinc-800"
    >
      <UIcon
        name="i-lucide-alert-circle"
        class="w-10 h-10 text-rose-400 mx-auto"
      />
    ...
    <template v-else>
      ...
      <div class="p-6 bg-white dark:bg-zinc-900 rounded-xl border border-zinc-200 dark:border-zinc-800 shadow-xs space-y-4">
        ...
        <div class="pt-3 border-t border-zinc-100 dark:border-zinc-800/80 flex flex-wrap gap-6 text-xs text-zinc-500">
          ...
          <div v-if="data.expired_on">
            Expires: <span class="font-medium text-zinc-300">{{ formatDate(data.expired_on) }}</span>
          </div>
        ...
        <div class="flex items-center justify-between">
          <div>
            <h2 class="text-base font-bold text-slate-900 dark:text-white">
              Click Traffic Over Time
            </h2>
            <p class="text-xs text-zinc-500">
              Visits grouped by {{ period }} intervals
            </p>
          </div>
          <UBadge
            color="neutral"
      ...
      <div class="p-6 bg-white dark:bg-zinc-900 rounded-xl border border-zinc-200 dark:border-zinc-800 space-y-4">
        <div class="flex items-center justify-between">
          <div>
            ...
            <p class="text-xs text-zinc-500">
              Latest 20 clicks recorded for this link
            </p>
          </div>
        </div>

        <div
          v-if="!data.recent_visitors.length"
          class="py-8 text-center text-xs text-zinc-500"
        >
            ...
            <tbody class="divide-y divide-zinc-800/60">
              ...
              >
                ...
                <td class="py-3 px-4 text-zinc-500">
                  {{ formatShortDate(visitor.created_at) }}
                </td>
              </tr>
            </tbody>
          </table>
